...
from pde import FieldCollection, PDEBase, plot_kymographs, ScalarField, CartesianGrid, FileStorage
...
class HowardPDE(PDEBase):
    ...
    def get_initial_state(self, grid):
        ...
        c_d = (ScalarField(grid, 1400, label="Field $c_D$")
               + 200 * ScalarField.random_normal(grid, label="Field $c_D$"))
...
if __name__ == '__main__':
    ...
    with NamedTemporaryFile(suffix=".hdf5") as path:
        ...
        writer = FileStorage(path.name, write_mode="truncate")

        eq.solve(state, 
            t_range=1000, 
            solver="scipy",
            backend="numba",
            tracker=["progress", writer.tracker(0.1)])

        # read the simulation back in again
        reader = FileStorage(path.name, write_mode="read_only")
