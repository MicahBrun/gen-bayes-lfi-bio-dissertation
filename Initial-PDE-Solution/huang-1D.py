...
if __name__ == '__main__':
    # initialize state
    grid = CartesianGrid([(0, 100)], [1028], [False])
    eq = HuangPDE(diffusivity=[2.5, 2.5], reaction_params=[1, 0.025, 0.0015, 0.0015, 0.093, 0.7], h=1)
    state = eq.get_initial_state(grid)

    # run a simulation
    file_write = FileStorage("./temp/huang-1D-16-5000-20260626-1443.hdf")

    eq.solve(state, 
        t_range=1000, 
        solver="scipy",
        backend="numba",
        tracker=["progress", file_write.tracker(1)])

    file_read = FileStorage("./temp/huang-1D-16-5000-20260626-1443.hdf", write_mode="read_only")
    plot_kymographs(file_read)